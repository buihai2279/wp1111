...
class monster_posts_carousel_widget extends WP_Widget {
    ...
    function widget( $args, $instance ) {
        ...
        <div class="tm-posts-carousel-widget" id="<?php echo $this->get_field_id('posts'); ?>">
            <?php
            $catid=$instance['cat'];
                $the_query = new WP_Query(array('cat'=>$catid,'orderby'=>'date','order'=>'DESC','posts_per_page'=>'4'));
...
function afterAction(){
    ...
    for (i = 0; i < tmp.length; i++ ) {
        ...
        // var t=$(".owl-stage-outer .owl-stage>.owl-item").length;
    }
}
    ...
    function form( $instance ) {
        ?>
        <?php $name= $this->get_field_name('cat');?>
        <?php $instance['cat'] = ! empty( $instance['cat'] ) ? $instance['cat'] : 0;
        $sel=$instance['cat']; ?>
